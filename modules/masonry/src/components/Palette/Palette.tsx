...
export function Palette({ config }: PaletteViewProps) {
  ...
  return (
    <div className="border-border bg-background text-foreground flex h-full min-h-0 w-full overflow-hidden rounded-lg border font-sans">
      ...
      <nav className="border-border bg-muted/40 flex w-18 shrink-0 flex-col gap-1 overflow-y-auto border-r px-1 pt-[104px] pb-2">
        {visibleCategories.map(({ category, index }) => {
          ...
          return (
            <Button
              ...
              className="h-auto flex-col gap-1 px-1 py-2 text-xs active:translate-y-0!"
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
          ...
          {classifications.map((classification, index) => {
            ...
            return (
              <Button
                ...
                className={cn(
                  'h-9 flex-1 active:translate-y-0!',
        ...
        {visibleCategories.length === 0 ? (
          ...
          <div
            className={cn(
              'flex-1 overflow-y-auto px-2 pt-3 pb-3 transition-[padding-bottom] duration-500',
              isDragging && 'pb-32',
            )}
          >
            {visibleCategories.map(({ category, index }) => {
